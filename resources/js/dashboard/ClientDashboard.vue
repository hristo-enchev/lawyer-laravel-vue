<template>
  <div>
    <div class="row">
      ...
      <div class="col-md-3">Search lawyer</div>
    </div>
    <div class="row">
      <div class="col-md-3">
...
<script>
import ClientAppointmentList from "./../appointments/ClientAppointmentList";
...
export default {
  ...
  methods: {
    getResults(page = 1) {
      axios
        .get(
          `/api/dashboard/client?page=${page}&status=${this.status}&order=${this.order}&sort=${this.sort}&search=${this.search}`
        )
        .then((responce) => {
